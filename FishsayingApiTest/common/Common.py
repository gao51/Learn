...
def get_xls(xls_name, sheet_name):
    ...
    k = 1
    for ti in sheet.row_values(0):
        if sheet.row_values(0)[k] == "execute":
            break
        else:
            k += 1
    for i in range(nrows):
        if sheet.row_values(i)[k] != u'execute' and str.lower(sheet.row_values(i)[k]) == "y":
...
def startFile():
    with open(proDir+"\Results", "w", encoding='utf-8') as fp:
        fp.write("")


def store(data):
    with open(proDir+"\Results", "a+", encoding='utf-8') as fp:
        fp.write(data + "\n")


def listdata():
    data=[]
    with open(proDir+"\Results", "r", encoding='utf-8') as fp:
        lines = fp.readlines()
        for line in lines:
            line_=eval(line)
            data.append(line_)
...
def replacebody(con):
    if "_${time}" in con:
        new = str(int(round(time.time() * 1000)))
        con = con.replace("_${time}", new)
    if "null" in con:
        con = con.replace("null","None")
    return con

def gethtml():
    with open(proDir+"//testFile//mailexcel.html" ,"r",encoding="utf-8") as fp:
        return fp.read()

if __name__ == "__main__":
    # print(get_xls("test1.xlsx", "Sheet1"))
    print(gethtml())